...
安全隐患域的统一「会员卡中心」：手机号短信验证码登录 + 时间制 VIP 会员（微信支付）+ 个人中心（跨产品历史聚合）。它本身不做识别等业务功能，是给安全隐患域各产品共用的「登录 + 付费 + 身份」底座。

谁在用 / 用在哪：A600 隐患识别、A800 安防文档库都依赖它登录和验 VIP——用户在那些产品里点登录会跳到这里，开通的 VIP 在全域通用。上线地址 `https://h100.jsai100.com/asg100/`（端口 4002）。代码最初从 A600 复制起步再改造，所以仓库里仍保留少量 A600 的痕迹（如 `reports` 表）。
...
# 至少填 ASG_MEMBER_SESSION_PASSWORD（48 字节随机串）
...
- **一张会员卡全域通用**：登录 cookie（`asg_member_session`）设 `Path=/`，A600 / A800 都能读到同一份登录态，实现跨产品免登 + VIP 共享。三个产品的 `ASG_MEMBER_SESSION_PASSWORD` 必须填同一个值。
...
- DB：`data/asg100.db`（better-sqlite3 + WAL），首次启动自动建表（users / reports / sms_codes / orders / memberships / membership_ledger / documents / document_downloads + 5 张 `rl_*` 限频表）。
...
- 部署清单见 `deploy/DEPLOY.md`（安全隐患域三产品一起上线的顺序、cookie 共享验证、回滚预案）。
