...
export async function exchangeCodeForOpenid(code) {
  if (!isOAuthReady()) {
    console.warn("[wechat-oauth] fake mode: env 不全。配齐 env 即可切真。");
    return { openid: `FAKE_OPENID_${Date.now().toString(36)}`, fakeMode: true };
  }
  if (code.startsWith("FAKE_CODE_")) {
    // 公众号 env 齐全（生产）时拒绝 FAKE code：防已登录用户手动调 callback 给自己塞假 openid
    throw new Error("FAKE_CODE 仅限本地 fake mode 使用");
  }
 ...
 * 同域判断用 ASG_ALLOWED_HOST（生产=h100.jsai100.com）；未配则只放行相对路径。
 */
export function isSafeFromPath(from) {
  ...
  if (/^https?:\/\//i.test(from)) {
    const allowedHost = process.env.ASG_ALLOWED_HOST;
 ...
 *  - 站内相对路径（/billing、/）→ 补上本产品子路径前缀 VITE_BASE_PATH（如 /asg100）
 *
 * 为什么必须补：部署在 /asg100/ 子路径时，res.redirect("/billing") 会被浏览器按根域名
 * 解析成 https://域名/billing，逃出 /asg100/，落到根路径的邻居应用（career-report）→ 404
 * 且显示那个应用的标题。前缀补回后稳留在 /asg100/billing。
 * VITE_BASE_PATH 未配（本地 base=/）时原样返回，本地不受影响。
 */
export function resolveRedirect(from) {
  if (/^https?:\/\//i.test(from)) return from;
  const prefix = (process.env.VITE_BASE_PATH || "/").replace(/\/+$/, ""); // "/asg100" 或 ""
