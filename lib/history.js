...
/**
 * 查"我的"文档下载记录（只读 A800 document_downloads）。
 * 库不存在或失败 → 返空（降级）。
 */
export function getDocDownloadHistory(phone, limit = 50) {
  ...
  try {
    ...
    console.error("[history] 读 A800 下载记录失败:", err?.message || err);
    return [];
  } finally {
    if (db) db.close();
  }
...
export function getHazardReportDetail(phone, reportId) {
  ...
  try {
    ...
    return {
      ...
      hazards,
      // 现场照片 data URL（老记录没存照片 → null）
      image: row.image_base64
        ? `data:${row.image_mime || "image/jpeg"};base64,${row.image_base64}`
        : null,
...
/**
 * 查"我的"最近 N 天识别报告（含完整隐患 + 原图 base64），给"下载台账"用。
 * 只读，按 created_at >= sinceTs 过滤。
 * @param {string} phone 当前登录手机号
 * @param {number} sinceTs 起始时间戳（含），早于此的不取
 * @returns {Array<{id,scenario,scenarioLabel,hazardCount,hazards,imageBase64,imageMime,createdAt}>}
 */
export function getRecentHazardReports(phone, sinceTs) {
  if (!phone) return [];
  if (!fs.existsSync(HAZARD_DB_PATH)) return [];
  let db;
  try {
    ...
    const rows = db
      .prepare(
        `SELECT id, scenario, scenario_label, hazard_count, report_json, created_at,
                image_base64, image_mime
         FROM reports WHERE user_phone = ? AND created_at >= ?
    ...
    return rows.map((r) => {
      let hazards = [];
      try {
        hazards = JSON.parse(r.report_json);
      } catch {
        hazards = [];
      }
      return {
        ...
        imageMime: r.image_mime || "image/jpeg",
        createdAt: r.created_at,
      };
    });
  } catch (err) {
    console.error("[history] 读 A600 近 N 天报告失败:", err?.message || err);
    return [];
  } finally {
    if (db) db.close();
  }
