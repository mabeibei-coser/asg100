...
// asg100 = 安全隐患域「会员中心」的数据库。
// 它只拥有会员域的表（账号 / 验证码 / 订单 / VIP / 流水 / 限频）。
// 识别记录归 A600 自己的库，文档归 A800 自己的库——中心不存业务数据，
// 要看就 ATTACH 只读聚合（见 lib/history.js）。
...
export function getDb() {
  ...
  // ── 6. rate-limiter-flexible 预建表（避免首次请求 race）──
  for (const name of [
    "rl_sms_code_phone",
    "rl_sms_code_ip",
    "rl_login_phone",
  ...
  // ── 7. site_settings（站点设置 key/value）──
  //   key='legal_terms' → 服务使用协议正文（HTML）；key='legal_privacy' → 隐私政策正文。
  //   schema 由 asg100 拥有；admin-hub「安防域协议」经授权跨库写入正文（见其 CLAUDE.md 铁律#1），
  //   登录页勾选项链接 + LegalView + 公开接口 GET /api/legal/:type 只读它。
  _db.exec(`
