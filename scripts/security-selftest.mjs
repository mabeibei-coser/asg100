// ASG100「钱与门」安全自测：验签/解密/下载token/开放重定向/充值幂等/源码级红线。
...
process.env.ASG_MEMBER_SESSION_PASSWORD ||= "selftest-password-32bytes-minimum!!";
// 临时库：测试只用临时路径，绝不指向真实业务库
const tmpDb = path.join(os.tmpdir(), `asg100-selftest-${process.pid}.db`);
process.env.ASG100_DB_PATH = tmpDb;
...
const apiV3Key = "0123456789abcdef0123456789abcdef"; // 32 字节测试 key
...
const makeNotify = ({ outTradeNo = "ASGTEST001", total = 3990, tsOffsetSec = 0, tamper = false, badSerial = false } = {}) => {
...
ok("正确签名的回调 → 通过且解出订单号/金额", good.ok === true && good.resource.outTradeNo === "ASGTEST001" && good.resource.raw?.amount?.total === 3990);
...
{
  const a = makeNotify(); const b = makeNotify({ outTradeNo: "ASGEVIL999" });
...
insOrder.run("ASGTEST001", "pkg_3m", 3990, 92, now, now);
const r1 = grantVipFromOrder("ASGTEST001");
const r2 = grantVipFromOrder("ASGTEST001");
ok("首次入账 applied=true", r1.applied === true);
ok("同订单重复回调 → 幂等命中不重复开通", r2.applied === false && r2.vipExpireAt === r1.vipExpireAt);
ok("会员流水只有 1 条", db.prepare("SELECT COUNT(*) c FROM membership_ledger WHERE related_order_id='ASGTEST001'").get().c === 1);
insOrder.run("ASGTEST002", "pkg_3m", 3990, 92, now, now);
const r3 = grantVipFromOrder("ASGTEST002");
